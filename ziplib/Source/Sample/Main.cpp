...
TEST_METHOD(Sample_UnzipFileAll)
{
	try
	{
		const char* my_archive = "MyArchive.zip";
		ZipArchive::Ptr archive = ZipFile::Open(my_archive);
		size_t entries = archive->GetEntriesCount();
		...
		for (size_t i = 0; i < entries; ++i) 
		{
			...
			printf("entry is Directory => %d", entry->IsDirectory());
			if (entry->IsDirectory()) {
				printf("[+] sorry, we could not unzip directory. \n");
			}
			else
			{
				const char* entryName = entry->GetFullName().c_str();
				ZipFile::ExtractFile(my_archive, entryName);
				printf("[+] Extracting '%s' as '%s' \n", entryName, entryName);
			}
		}
